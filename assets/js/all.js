...
var tickets = [{
  ...
  num: 8,
  rank: 8.6,
  describe: "嚴選超高CP值綠島自由行套裝行程，多種綠島套裝組合，提供台東綠島來回船票、綠島環島機車、綠島民宿住宿，行程加贈『綠島浮潛體驗』以及『綠島生態導覽』，讓你用輕鬆的綠島套裝自由行，也能深度認識綠島在地文化。"
}, {
  id: 2,
  name: "南庄度假村露營車二日遊",
  ...
  num: 2,
  rank: 9.2,
  describe: "南庄雲水豪華露營車，快來擁有最愜意的露營體驗吧！ 一泊一食，輕鬆享受露營車樂趣。 獨立衛浴與私人戶外露臺。 入住豪華露營車還能使用戶外SPA大眾湯，感受美人湯魅力。"
}, {
  id: 3,
  name: "漁樂碼頭釣魚體驗套票",
  ...
  num: 5,
  rank: 9.2,
  describe: "台中全新親子景點寶熊漁樂碼頭，為知名釣具公司「OKUMA」所創立的觀光工廠。一樓藍白希臘漁村風商店街免費參觀。二樓釣魚故事館則設立全台唯一虛擬釣場，透過導覽讓你知道如何釣魚、魚餌怎麼區分，寓教於樂的台中景點！"
}];
var btnAddTicket = document.querySelector("#btnAddTicket"); // 新增套票
...
btnAddTicket.addEventListener("click", function () {
  ...
  var scenicArea = document.querySelector("#scenicArea");
  ...
  var ticketDescribe = document.querySelector("#ticketDescribe");
  var ticketData = {
    id: tickets.length + 1,
    ...
    num: Number(ticketNum.value),
    rank: Number(ticketRank.value),
    describe: ticketDescribe.value
  };
  var isTicketData = ticketName.value !== "" && ticketImgUrl.value !== "" && scenicArea.value !== "" && ticketPrice.value !== "" && ticketNum.value !== "" && ticketRank.value !== "" && ticketDescribe.value !== ""; // 有輸入套票訊息才能新增
...
searchArea.addEventListener("change", function (e) {
  var area = e.target.value; // 當選則全部地區，則顯示套票陣列
